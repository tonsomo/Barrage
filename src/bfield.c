...
extern SDL_Surface *img_trees, *background, *img_ground, *img_small_crater, *img_crater, *img_gun;
extern void vec2xy( Vector *v, int *sx, int *sy ); /* from shots.c */
extern SDL_Sound *wav_expl1, *wav_expl2, *wav_cannon1, *wav_cannon2;
...
void bfield_init( int type, Vector *gun, Vector *gun_img_offset )
{
	...
	/* basic ground */
	SDL_BlitSurface( img_ground, 0, background, 0 );
...
static void bfield_draw_gun( SDL_Surface *dest, int camera_x, int camera_y )
{
	...
	SDL_BlitSurface( img_gun, &srect, dest, &drect );
}
...
void bfield_draw( SDL_Surface *dest, int camera_x, int camera_y )
{
	...
	SDL_BlitSurface( background, 0, dest, 0 );
	bfield_draw_gun( dest, camera_x, camera_y );
	...
	for ( i = 0; i < bfield.tree_count; i++ ) {
		...
		SDL_BlitSurface( tree->img, &srect, dest, &drect );
	}
	
	particles_draw( dest, 0, 0 );
	shots_draw( dest, 0, 0 );
}
...
void bfield_fire_gun( int type /* ST_... */, Vector *dest, double alpha, double power )
{
	...
	if ( dest->x < 70 && abs(dest->y-bfield.gun.y) < 50 )
		return;
...
static void trees_check_impact( int x, int y, int radius )
{
	Tree *tree;
	int i;
	...
	for ( i = 0; i < bfield.tree_count; i++ ) {
		...
		if ( DIST( tree->x+tree->hx, tree->y+tree->hy, x, y ) < radius ) {
			/* it burns! */
			particles_add_emitter( PT_FIRE,
					tree->x+tree->hx, tree->y+tree->hy,
					5, 0.1, 10000 );
			tree->burn_time = 10000;
		}
	}
}
...
void bfield_handle_impact( Shot *shot )
{
	...
	switch ( shot->type ) {
		...
		drect.x = sx - (img_small_crater->w>>1);
		drect.y = sy - (img_small_crater->h>>1);
		drect.w = img_small_crater->w; drect.h = img_small_crater->h;
		SDL_BlitSurface( img_small_crater, 0, background, &drect );

		SDL_PlaySound( wav_expl1 );
		
		units_check_impact( sx, sy, 20 );
		trees_check_impact( sx, sy, 20 );
		break;
	    case ST_BOMB:
		particles_explode_bomb( sx, sy );
		drect.x = sx - (img_crater->w>>1);
		drect.y = sy - (img_crater->h>>1);
		drect.w = img_crater->w; drect.h = img_crater->h;
		SDL_BlitSurface( img_crater, 0, background, &drect );
			
		SDL_PlaySound( wav_expl2 );

		units_check_impact( sx, sy, 40 );
		trees_check_impact( sx, sy, 30 );
		break;
	    case ST_CLUSTER:
