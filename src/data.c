...
SDL_Sound *wav_scream1 = 0;
SDL_Sound *wav_scream2 = 0; /* human screams */
...
static SDL_Surface *data_load_image( char *name )
{
	...
	snprintf( path, 128, "%s/gfx/%s", ".", name ); path[127] = 0;
...
static SDL_Sound *data_load_sound( char *name )
{
	char path[128];
	SDL_Sound *wav = 0;
	snprintf( path, 128, "%s/sounds/%s", ".", name ); path[127] = 0;
	printf( "loading %s ... ", path );
...
void data_load()
{
	...
	wav_scream1 = data_load_sound( "scream1.wav" );
...
void data_delete()
{
	...
	data_free_sound( &wav_scream1 );
	data_free_sound( &wav_scream2 );
